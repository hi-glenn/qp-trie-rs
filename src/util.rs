...
pub fn nybble_index(n: usize, slice: &[u8]) -> u8 {
    ...
    if byte_idx < slice.len() {
        ...
        // array.
        if n & 1 == 0 {
            libc_print::libc_println!("nybble_index(); n 为 偶数: 0b{:04b}; nybble_mismatch: {};", 1 + (byte & 0x0F), n);
            1 + (byte & 0x0F)
        } else {
            libc_print::libc_println!("nybble_index(); n 为 奇数: 0b{:04b}; nybble_mismatch: {};", 1 + (byte >> 4), n);
            1 + (byte >> 4)
        }
    } else {
        ...
        libc_print::libc_println!("nybble_index(); 🐷 0; nybble_mismatch: {};", n);
        0
    }
}
